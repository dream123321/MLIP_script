...
class DFTSUS2Comparator:
    ...
    def compute_all_metrics(self, dft, sus):
        ...
        r2_f_vec = r2_score(f_dft_vec, f_sus_vec)
    ...
    def create_combined_plot(self, dft_file, sus_file):
        ...
        创建一行三列的综合图：能量、力、应力
        """
        print(f"Loading data: DFT={dft_file}, SUS2={sus_file}")
        ...
        # 子图 (c): 应力
        sc_stress = self._plot_stress_subfigure(
            axs[2],
    ...
    def _plot_energy_subfigure(self, ax, dft_data, sus_data, mae, rmse, r2):
        """绘制能量子图"""
    ...
    def _plot_force_subfigure(self, ax, force_metrics, force_mode):
        """绘制力子图"""
        if force_mode == 'magnitude':
            # 模式1：显示合力大小
            return self._plot_force_magnitude(ax, force_metrics)
        elif force_mode == 'components':
            # 模式2：显示力分量（在同一张图上，使用密度颜色）
            return self._plot_force_components(ax, force_metrics)
        else:
            raise ValueError(f"Unknown force mode: {force_mode}")

    def _plot_force_magnitude(self, ax, force_metrics):
        """绘制合力大小子图"""
        ...
        ax.text(0.03, 0.98, '(b)', transform=ax.transAxes,
                fontsize=40, fontweight='bold', va='top')
        ax.set_aspect('equal')

        return sc

    def _plot_force_components(self, ax, force_metrics):
    ...
    def _plot_stress_subfigure(self, ax, dft_data, sus_data, mae, rmse, r2):
        """绘制应力子图"""
        sc = self.density_scatter(ax, dft_data, sus_data)
        ...
        ax.set_ylabel('SUS\u00B2 Stress (GPa)', fontweight='bold')
        ax.text(0.03, 0.98, '(c)', transform=ax.transAxes,
                fontsize=40, fontweight='bold', va='top')
        ax.set_aspect('equal')
...
示例:
  ...
  python dft_sus2_compare.py dft_data.cfg sus2_data.cfg (cfg转xyz这里会随机分配元素，但不影响结果。不输入正确的元素顺序的临时的xyz无作用，默认会删除)
  python dft_sus2_compare.py dft_data.xyz sus2_data.xyz
  python dft_sus2_compare.py dft_data.cfg sus2_data.xyz 

  # 多元素系统（如Na-Ca-Ti-O）
  python dft_sus2_compare.py dft.cfg sus2.cfg --elements Na Ca Ti O

  # 显示力分量（Fx, Fy, Fz）
  python dft_sus2_compare.py dft.cfg sus2.xyz --force-mode components

  # 保留临时文件
  python dft_sus2_compare.py dft.cfg sus2.cfg --keep-temp True

  # 自定义参数
  python dft_sus2_compare.py dft.cfg sus2.cfg --num-processes 32 --output comparison.jpg
  python dft_sus2_compare.py dft.cfg sus2.cfg --force-mode components --save-data True
        """
    )
    ...
    # 新增参数：CFG文件相关
    parser.add_argument('--elements', type=str, nargs='+', default=['H', 'Li', 'Be', 'B', 'C', 'N', 'O', 'F', 'Na', 'Mg', 'Al', 'Si', 'P', 'S', 'Cl', 'K', 'Ca',
                   'Sc', 'Ti', 'V', 'Cr', 'Mn', 'Fe', 'Co', 'Ni', 'Cu', 'Zn', 'Ga', 'Ge', 'As', 'Se', 'Br', 'Rb',
                   'Sr', 'Y', 'Zr', 'Nb', 'Mo', 'Tc', 'Ru', 'Rh', 'Pd', 'Ag', 'Cd', 'In', 'Sn', 'Sb', 'Te', 'I',
                   'Cs', 'Ba', 'La', 'Ce', 'Pr', 'Nd', 'Pm', 'Sm', 'Eu', 'Gd', 'Tb', 'Dy', 'Ho', 'Er', 'Tm', 'Yb',
                   'Lu', 'Hf', 'Ta', 'W', 'Re', 'Os', 'Ir', 'Pt', 'Au', 'Hg', 'Tl', 'Pb', 'Bi', 'Ac', 'Th', 'Pa',
    ...
    parser.add_argument('--version', action='version', version='DFT-SUS2 Comparator v1.1 (with CFG support)')

    # 解析参数
    args = parser.parse_args()

    print("=" * 60)
    print("DFT-SUS2 COMPARATOR (with CFG support)")
    print("=" * 60)
    print(f"DFT data: {args.dft_file}")
    print(f"SUS2 data: {args.sus_file}")
