...
class MDWorkflowManager:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.default_config = {
            ...
            # 元素和体系配置
            'analysics_specie': 'Na',
            'elements': ['Na', 'S', 'Ti', 'Zn', 'Zr', 'Hf'],  #势函数对应的元素
            'supercell_size': (3, 4, 3),
            # MSD元素配置（可选）
            ...
            'nvt_steps': 350000,  # NVT步数
            'dump_frequency': 35000,  # 输出轨迹结构频率
            # 模拟统计参数（可选）
            'ave_params': None,  # 统计平均参数，默认None

            # 温度设置
            'temperatures': [1000,900,800,700,600,500,400,300],  # 温度列表 (K)
            'repeat_num': 3,  # 重复模拟次数
