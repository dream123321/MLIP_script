...
def process_all_replicates(grouped_files, start_proportion, end_proportion, time_step, dimension, specie,dic_ele_valence):
...
def calculate_room_temp_conductivity_from_D(D_all, V, N, specie, dic_ele_valence,T_pred=298):
    ...
    for D in D_all:
        ...
        temp.append(sigma)
    all_pred, all_lower, all_upper, mean_pred, mean_lower, mean_upper = temp[0],temp[1],temp[2],temp[3],temp[4],temp[5]
...
def list_diffusion_with_average(grouped_files, start_proportion, end_proportion, out_path, specie='Li',
    ...
    all_results = process_all_replicates(grouped_files, start_proportion, end_proportion, time_step, dimension, specie,dic_ele_valence)
...
def main_analysics(input_dir, room_temp=298, specie='Li', time_step=1e-15,
                   start_proportion=0.3, end_proportion=0.0, dic_ele_valence=None):
    ...
    if len(all_results) >= 2:
        ...
        # 计算室温扩散系数
        D_all = all_pred,all_lower,all_upper,mean_pred,mean_lower,mean_upper = calculate_room_temp_diffusivity_with_uncertainty(D_fit_result, T_pred=room_temp)
        ...
        # 方法1：从扩散系数计算室温离子电导率
        sigma_NE = calculate_room_temp_conductivity_from_D(D_all, V_lowest, N_lowest, specie, T_pred=room_temp,dic_ele_valence=dic_ele_valence)
        print(
        ...
        sigma_all, sigma_all_lower, sigma_all_upper, sigma_mean, sigma_mean_lower, sigma_mean_upper = calculate_room_temp_conductivity_from_fit(sigma_fit_result, T_pred=room_temp)
        print(
        ...
        with open(extrapolation_file, 'w') as f:
            ...
            f.write(f"The OLS (Ordinary Least Squares) regression results denoted with an asterisk (***) is derived from all individual measurements. (Perhaps more accurate!)\n"
